...
export default function SearchBar({ searchList, backUrl }: Props) {
  ...
  return (
    <>
      <label className="relative mx-auto block max-w-[90vw] md:max-w-4xl">
        <span className="sr-only">Search</span>
        <span className="absolute inset-y-0 left-0 flex items-center pl-2 opacity-75">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            aria-hidden="true"
            className="fill-foreground inline-block size-6"
          >
      ...
      <ul className="mx-auto grid max-w-[80vw] grid-cols-1 gap-8 md:max-w-4xl md:grid-cols-2">
        {searchResults &&
          searchResults.map(({ item, refIndex }) => (
            <Card
              href={`/blog/${item.slug}${fullBackUrl}`}
