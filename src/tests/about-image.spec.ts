...
test.describe("About page images", () => {
  test("should load profile image and FCC certificates", async ({ page }) => {
    ...
    if (profileSrc) {
      const imageResponse = await page.request.get(
        `http://localhost:4321${profileSrc}`
      );
    ...
    const fccImages = page
      .locator("img")
      .filter({ hasNot: page.locator('img[alt="Avatar image"]') });
    ...
    for (let i = 0; i < fccImageCount; i++) {
      ...
      if (fccSrc) {
        const imageResponse = await page.request.get(
          `http://localhost:4321${fccSrc}`
        );
        console.log(
          `FCC image ${i + 1} response status:`,
  ...
  test("should show proper error handling for broken images", async ({
    page,
  }) => {
