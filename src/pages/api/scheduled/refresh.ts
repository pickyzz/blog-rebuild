...
export const GET: APIRoute = async ({ url }) => {
  try {
    ...
    console.log("[SCHEDULED REFRESH] All caches invalidated via scheduled job at", new Date().toISOString());

    return new Response(JSON.stringify({
      success: true,
      message: "Cache refreshed via scheduled job",
      timestamp: new Date().toISOString(),
      nextSuggestedRefresh: new Date(Date.now() + 5 * 60 * 1000).toISOString() // 5 minutes from now
    }), {
      status: 200,
      headers: { "Content-Type": "application/json" }
    });
